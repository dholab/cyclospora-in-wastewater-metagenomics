...
This stage turns public reference sequence into the bait set used to screen wastewater libraries in [stage 02](../02-screen-wastewater-metagenomes/): 1,184 canonical 31-mers that occur in *C. cayetanensis* mature ribosomal RNA, occur in no other ribosomal RNA in SILVA or Rfam, occur in no non-*cayetanensis Cyclospora* reference, and have no exact full-length match anywhere in NCBI `core_nt` assigned to another taxon.
...
```bash
pixi install
pixi run test       # 89 unit tests and 3 fixture builds; no network, no prior build needed
pixi run build      # design the bait set from pinned references      -> 1,388 baits
pixi run validate   # apply the committed core_nt evidence            -> 1,184 baits
```
...
| [`core_nt_bait_exact_match_blast.tsv`](results/core_nt_bait_exact_match_blast.tsv) | 5,012 | Raw exact-match hits from the `core_nt` search, with subject accession, taxid, and title. This is the evidence `pixi run validate` consumes. |
